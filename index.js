...
class Pro54PatchView extends HTMLElement
{
    constructor (patchConnection)
    {
        ...
        this.patchConnection.addEndpointListener ("midiIn", message =>
        {
            ...
            const rawMidiMessage = message.message;

            this.keyboardElement?.handleExternalMIDI (rawMidiMessage);
            this.modWheelElement?.handleExternalMIDI (rawMidiMessage);
            this.pitchBendElement?.handleExternalMIDI (rawMidiMessage);
            this.midiBlink?.handleExternalMIDI (rawMidiMessage);

            // --- chuma: NEW LOGIC for CC70 and CC71 ---
            const statusByte = (rawMidiMessage >> 16) & 0xff; // MIDI messages are 24-bit in Cmajor: 00 SS DD VV
            const msgType = statusByte & 0xf0;
            const ccNumber = (rawMidiMessage >> 8) & 0xff;
            const ccValue = rawMidiMessage & 0xff; // MIDI value 0-127

            if (msgType === 0xb0) // Control Change
            {
                if (ccNumber === 70) // Filter Cutoff
                {
                    const cutoffKnob = this.shadowRoot.getElementById("FilterCutoff");
                    if (cutoffKnob)
                    {
                        const scaledValue = (ccValue / 127.0) * 100.0;
                        cutoffKnob.setCurrentValue?.(scaledValue);
                    }
                }
                else if (ccNumber === 71) // Filter Resonance
                {
                    const resonanceKnob = this.shadowRoot.getElementById("FilterResonance");
                    if (resonanceKnob)
                    {
                        const scaledValue = (ccValue / 127.0) * 100.0;
                        resonanceKnob.setCurrentValue?.(scaledValue);
                    }
                }
                else if (ccNumber === 20) // PolyModFilterEnv
                {
                    const knob = this.shadowRoot.getElementById("PolyModFilterEnv");
                    if (knob) { const scaledValue = (ccValue / 127.0) * 100.0; knob.setCurrentValue?.(scaledValue); }
                }
                else if (ccNumber === 21) // PolyModOscB
                {
                    const knob = this.shadowRoot.getElementById("PolyModOscB");
                    if (knob) { const scaledValue = (ccValue / 127.0) * 100.0; knob.setCurrentValue?.(scaledValue); }
                }
                else if (ccNumber === 40) // OscAFreq
                {
                    const knob = this.shadowRoot.getElementById("OscAFreq");
                    if (knob) { const scaledValue = (ccValue / 127.0) * 100.0; knob.setCurrentValue?.(scaledValue); }
                }
                else if (ccNumber === 43) // OscAPW
                {
                    const knob = this.shadowRoot.getElementById("OscAPW");
                    if (knob) { const scaledValue = (ccValue / 127.0) * 100.0; knob.setCurrentValue?.(scaledValue); }
                }
                else if (ccNumber === 45) // MixerOscALevel
                {
                    const knob = this.shadowRoot.getElementById("MixerOscALevel");
                    if (knob) { const scaledValue = (ccValue / 127.0) * 100.0; knob.setCurrentValue?.(scaledValue); }
                }
                else if (ccNumber === 46) // MixerOscBLevel
                {
                    const knob = this.shadowRoot.getElementById("MixerOscBLevel");
                    if (knob) { const scaledValue = (ccValue / 127.0) * 100.0; knob.setCurrentValue?.(scaledValue); }
                }
                else if (ccNumber === 47) // MixerNoiseLevel
                {
                    const knob = this.shadowRoot.getElementById("MixerNoiseLevel");
                    if (knob) { const scaledValue = (ccValue / 127.0) * 100.0; knob.setCurrentValue?.(scaledValue); }
                }
                else if (ccNumber === 48) // ExternalInputLevel
                {
                    const knob = this.shadowRoot.getElementById("ExternalInputLevel");
                    if (knob) { const scaledValue = (ccValue / 127.0) * 100.0; knob.setCurrentValue?.(scaledValue); }
                }
                else if (ccNumber === 72) // FilterEnvAmt
                {
                    const knob = this.shadowRoot.getElementById("FilterEnvAmt");
                    if (knob) { const scaledValue = (ccValue / 127.0) * 100.0; knob.setCurrentValue?.(scaledValue); }
                }
                else if (ccNumber === 73) // FilterKeyboardTracking
                {
                    const knob = this.shadowRoot.getElementById("FilterKeyboardTracking");
                    if (knob) { const scaledValue = (ccValue / 127.0) * 100.0; knob.setCurrentValue?.(scaledValue); }
                }
                else if (ccNumber === 75) // FilterAttack
                {
                    const knob = this.shadowRoot.getElementById("FilterAttack");
                    if (knob) { const scaledValue = (ccValue / 127.0) * 100.0; knob.setCurrentValue?.(scaledValue); }
                }
                else if (ccNumber === 76) // FilterDecay
                {
                    const knob = this.shadowRoot.getElementById("FilterDecay");
                    if (knob) { const scaledValue = (ccValue / 127.0) * 100.0; knob.setCurrentValue?.(scaledValue); }
                }
                else if (ccNumber === 77) // FilterSustain
                {
                    const knob = this.shadowRoot.getElementById("FilterSustain");
                    if (knob) { const scaledValue = (ccValue / 127.0) * 100.0; knob.setCurrentValue?.(scaledValue); }
                }
                else if (ccNumber === 78) // FilterRelease
                {
                    const knob = this.shadowRoot.getElementById("FilterRelease");
                    if (knob) { const scaledValue = (ccValue / 127.0) * 100.0; knob.setCurrentValue?.(scaledValue); }
                }
                else if (ccNumber === 105) // DelayTime
                {
                    const knob = this.shadowRoot.getElementById("DelayTime");
                    if (knob) { const scaledValue = (ccValue / 127.0) * 100.0; knob.setCurrentValue?.(scaledValue); }
                }
                else if (ccNumber === 106) // DelaySpread
                {
                    const knob = this.shadowRoot.getElementById("DelaySpread");
                    if (knob) { const scaledValue = (ccValue / 127.0) * 100.0; knob.setCurrentValue?.(scaledValue); }
                }
                else if (ccNumber === 107) // DelayDepth
                {
                    const knob = this.shadowRoot.getElementById("DelayDepth");
                    if (knob) { const scaledValue = (ccValue / 127.0) * 100.0; knob.setCurrentValue?.(scaledValue); }
                }
                else if (ccNumber === 108) // DelayRate
                {
                    const knob = this.shadowRoot.getElementById("DelayRate");
                    if (knob) { const scaledValue = (ccValue / 127.0) * 100.0; knob.setCurrentValue?.(scaledValue); }
                }
                else if (ccNumber === 110) // DelayFeedback
                {
                    const knob = this.shadowRoot.getElementById("DelayFeedback");
                    if (knob) { const scaledValue = (ccValue / 127.0) * 100.0; knob.setCurrentValue?.(scaledValue); }
                }
                else if (ccNumber === 112) // DelayHiCut
                {
                    const knob = this.shadowRoot.getElementById("DelayHiCut");
                    if (knob) { const scaledValue = (ccValue / 127.0) * 100.0; knob.setCurrentValue?.(scaledValue); }
                }
                else if (ccNumber === 111) // DelayLoCut
                {
                    const knob = this.shadowRoot.getElementById("DelayLoCut");
                    if (knob) { const scaledValue = (ccValue / 127.0) * 100.0; knob.setCurrentValue?.(scaledValue); }
                }
                else if (ccNumber === 116) // DelayWet
                {
                    const knob = this.shadowRoot.getElementById("DelayWet");
                    if (knob) { const scaledValue = (ccValue / 127.0) * 100.0; knob.setCurrentValue?.(scaledValue); }
                }
                else if (ccNumber === 26) // LfoFrequency
                {
                    const knob = this.shadowRoot.getElementById("LfoFrequency");
                    if (knob) { const scaledValue = (ccValue / 127.0) * 100.0; knob.setCurrentValue?.(scaledValue); }
                }
                else if (ccNumber === 50) // OscBFreq
                {
                    const knob = this.shadowRoot.getElementById("OscBFreq");
                    if (knob) { const scaledValue = (ccValue / 127.0) * 100.0; knob.setCurrentValue?.(scaledValue); }
                }
                else if (ccNumber === 51) // OscBFreqFine
                {
                    const knob = this.shadowRoot.getElementById("OscBFreqFine");
                    if (knob) { const scaledValue = (ccValue / 127.0) * 100.0; knob.setCurrentValue?.(scaledValue); }
                }
                else if (ccNumber === 55) // OscBPWAmount
                {
                    const knob = this.shadowRoot.getElementById("OscBPWAmount");
                    if (knob) { const scaledValue = (ccValue / 127.0) * 100.0; knob.setCurrentValue?.(scaledValue); }
                }
                else if (ccNumber === 34) // WheelModulationLfoNoise
                {
                    const knob = this.shadowRoot.getElementById("WheelModulationLfoNoise");
                    if (knob) { const scaledValue = (ccValue / 127.0) * 100.0; knob.setCurrentValue?.(scaledValue); }
                }
                else if (ccNumber === 5) // Glide
                {
                    const knob = this.shadowRoot.getElementById("Glide");
                    if (knob) { const scaledValue = (ccValue / 127.0) * 100.0; knob.setCurrentValue?.(scaledValue); }
                }
                else if (ccNumber === 80) // AmplifierAttack
                {
                    const knob = this.shadowRoot.getElementById("AmplifierAttack");
                    if (knob) { const scaledValue = (ccValue / 127.0) * 100.0; knob.setCurrentValue?.(scaledValue); }
                }
                else if (ccNumber === 81) // AmplifierDecay
                {
                    const knob = this.shadowRoot.getElementById("AmplifierDecay");
                    if (knob) { const scaledValue = (ccValue / 127.0) * 100.0; knob.setCurrentValue?.(scaledValue); }
                }
                else if (ccNumber === 82) // AmplifierSustain
                {
                    const knob = this.shadowRoot.getElementById("AmplifierSustain");
                    if (knob) { const scaledValue = (ccValue / 127.0) * 100.0; knob.setCurrentValue?.(scaledValue); }
                }
                else if (ccNumber === 83) // AmplifierRelease
                {
                    const knob = this.shadowRoot.getElementById("AmplifierRelease");
                    if (knob) { const scaledValue = (ccValue / 127.0) * 100.0; knob.setCurrentValue?.(scaledValue); }
                }
                else if (ccNumber === 87) // Analog
                {
                    const knob = this.shadowRoot.getElementById("Analog");
                    if (knob) { const scaledValue = (ccValue / 127.0) * 100.0; knob.setCurrentValue?.(scaledValue); }
                }
                else if (ccNumber === 88) // MasterTune
                {
                    const knob = this.shadowRoot.getElementById("MasterTune");
                    if (knob) { const scaledValue = (ccValue / 127.0) * 100.0; knob.setCurrentValue?.(scaledValue); }
                }
                else if (ccNumber === 7) // Volume
                {
                    const knob = this.shadowRoot.getElementById("Volume");
                    if (knob) { const scaledValue = (ccValue / 127.0) * 100.0; knob.setCurrentValue?.(scaledValue); }
                }
                
                // --- Buttons ---
                else if (ccNumber === 41) // OscASaw
                {
                    const button = this.shadowRoot.getElementById("OscASaw");
                    if (button) { const isOn = (ccValue >= 64); button.setCurrentValue?.(isOn ? 1.0 : 0.0); }
                }
                else if (ccNumber === 22) // PolyModFreqA
                {
                    const button = this.shadowRoot.getElementById("PolyModFreqA");
                    if (button) { const isOn = (ccValue >= 64); button.setCurrentValue?.(isOn ? 1.0 : 0.0); }
                }
                else if (ccNumber === 23) // PolyModPWA
                {
                    const button = this.shadowRoot.getElementById("PolyModPWA");
                    if (button) { const isOn = (ccValue >= 64); button.setCurrentValue?.(isOn ? 1.0 : 0.0); }
                }
                else if (ccNumber === 24) // PolyModFilt
                {
                    const button = this.shadowRoot.getElementById("PolyModFilt");
                    if (button) { const isOn = (ccValue >= 64); button.setCurrentValue?.(isOn ? 1.0 : 0.0); }
                }
                else if (ccNumber === 42) // OscAPulse
                {
                    const button = this.shadowRoot.getElementById("OscAPulse");
                    if (button) { const isOn = (ccValue >= 64); button.setCurrentValue?.(isOn ? 1.0 : 0.0); }
                }
                else if (ccNumber === 44) // OscASync
                {
                    const button = this.shadowRoot.getElementById("OscASync");
                    if (button) { const isOn = (ccValue >= 64); button.setCurrentValue?.(isOn ? 1.0 : 0.0); }
                }
                else if (ccNumber === 113) // DelayINV
                {
                    const button = this.shadowRoot.getElementById("DelayINV");
                    if (button) { const isOn = (ccValue >= 64); button.setCurrentValue?.(isOn ? 1.0 : 0.0); }
                }
                else if (ccNumber === 115) // DelayON
                {
                    const button = this.shadowRoot.getElementById("DelayON");
                    if (button) { const isOn = (ccValue >= 64); button.setCurrentValue?.(isOn ? 1.0 : 0.0); }
                }
                else if (ccNumber === 117) // DelaySync
                {
                    const button = this.shadowRoot.getElementById("DelaySync");
                    if (button) { const isOn = (ccValue >= 64); button.setCurrentValue?.(isOn ? 1.0 : 0.0); }
                }
                else if (ccNumber === 118) // DelayMidi
                {
                    const button = this.shadowRoot.getElementById("DelayMidi");
                    if (button) { const isOn = (ccValue >= 64); button.setCurrentValue?.(isOn ? 1.0 : 0.0); }
                }
                else if (ccNumber === 25) // LfoMidiSync
                {
                    const button = this.shadowRoot.getElementById("LfoMidiSync");
                    if (button) { const isOn = (ccValue >= 64); button.setCurrentValue?.(isOn ? 1.0 : 0.0); }
                }
                else if (ccNumber === 27) // LfoShapeSaw
                {
                    const button = this.shadowRoot.getElementById("LfoShapeSaw");
                    if (button) { const isOn = (ccValue >= 64); button.setCurrentValue?.(isOn ? 1.0 : 0.0); }
                }
                else if (ccNumber === 28) // LfoShapeTri
                {
                    const button = this.shadowRoot.getElementById("LfoShapeTri");
                    if (button) { const isOn = (ccValue >= 64); button.setCurrentValue?.(isOn ? 1.0 : 0.0); }
                }
                else if (ccNumber === 29) // LfoShapePulse
                {
                    const button = this.shadowRoot.getElementById("LfoShapePulse");
                    if (button) { const isOn = (ccValue >= 64); button.setCurrentValue?.(isOn ? 1.0 : 0.0); }
                }
                else if (ccNumber === 52) // OscBShapeSaw
                {
                    const button = this.shadowRoot.getElementById("OscBShapeSaw");
                    if (button) { const isOn = (ccValue >= 64); button.setCurrentValue?.(isOn ? 1.0 : 0.0); }
                }
                else if (ccNumber === 53) // OscBShapeTri
                {
                    const button = this.shadowRoot.getElementById("OscBShapeTri");
                    if (button) { const isOn = (ccValue >= 64); button.setCurrentValue?.(isOn ? 1.0 : 0.0); }
                }
                else if (ccNumber === 54) // OscBShapePulse
                {
                    const button = this.shadowRoot.getElementById("OscBShapePulse");
                    if (button) { const isOn = (ccValue >= 64); button.setCurrentValue?.(isOn ? 1.0 : 0.0); }
                }
                else if (ccNumber === 56) // OscBSubOsc
                {
                    const button = this.shadowRoot.getElementById("OscBSubOsc");
                    if (button) { const isOn = (ccValue >= 64); button.setCurrentValue?.(isOn ? 1.0 : 0.0); }
                }
                else if (ccNumber === 57) // OscBKKeyboardTracking
                {
                    const button = this.shadowRoot.getElementById("OscBKKeyboardTracking");
                    if (button) { const isOn = (ccValue >= 64); button.setCurrentValue?.(isOn ? 1.0 : 0.0); }
                }
                else if (ccNumber === 35) // WheelModulationFreqOscA
                {
                    const button = this.shadowRoot.getElementById("WheelModulationFreqOscA");
                    if (button) { const isOn = (ccValue >= 64); button.setCurrentValue?.(isOn ? 1.0 : 0.0); }
                }
                else if (ccNumber === 36) // WheelModulationFreqOscB
                {
                    const button = this.shadowRoot.getElementById("WheelModulationFreqOscB");
                    if (button) { const isOn = (ccValue >= 64); button.setCurrentValue?.(isOn ? 1.0 : 0.0); }
                }
                else if (ccNumber === 37) // WheelModulationPWA
                {
                    const button = this.shadowRoot.getElementById("WheelModulationPWA");
                    if (button) { const isOn = (ccValue >= 64); button.setCurrentValue?.(isOn ? 1.0 : 0.0); }
                }
                else if (ccNumber === 38) // WheelModulationPWB
                {
                    const button = this.shadowRoot.getElementById("WheelModulationPWB");
                    if (button) { const isOn = (ccValue >= 64); button.setCurrentValue?.(isOn ? 1.0 : 0.0); }
                }
                else if (ccNumber === 39) // WheelModulationFilter
                {
                    const button = this.shadowRoot.getElementById("WheelModulationFilter");
                    if (button) { const isOn = (ccValue >= 64); button.setCurrentValue?.(isOn ? 1.0 : 0.0); }
                }
                else if (ccNumber === 59) // Unison
                {
                    const button = this.shadowRoot.getElementById("Unison");
                    if (button) { const isOn = (ccValue >= 64); button.setCurrentValue?.(isOn ? 1.0 : 0.0); }
                }
                else if (ccNumber === 85) // Release
                {
                    const button = this.shadowRoot.getElementById("Release");
                    if (button) { const isOn = (ccValue >= 64); button.setCurrentValue?.(isOn ? 1.0 : 0.0); }
                }
                else if (ccRgbNumber === 86) // Velocity
                {
                    const button = this.shadowRoot.getElementById("Velocity");
                    if (button) { const isOn = (ccValue >= 64); button.setCurrentValue?.(isOn ? 1.0 : 0.0); }
                }
                else if (ccNumber === 30) // Repeat
                {
                    const button = this.shadowRoot.getElementById("Repeat");
                    if (button) { const isOn = (ccValue >= 64); button.setCurrentValue?.(isOn ? 1.0 : 0.0); }
                }
                else if (ccNumber === 84) // Drone
                {
                    const button = this.shadowRoot.getElementById("Drone");
                    if (button) { const isOn = (ccValue >= 64); button.setCurrentValue?.(isOn ? 1.0 : 0.0); }
                }
                else if (ccNumber === 74) // FilterHPF
                {
                    const button = this.shadowRoot.getElementById("FilterHPF");
                    if (button) { const isOn = (ccValue >= 64); button.setCurrentValue?.(isOn ? 1.0 : 0.0); }
                }
                else if (ccNumber === 79) // FilterInvertEnv
                {
                    const button = this.shadowRoot.getElementById("FilterInvertEnv");
                    if (button) { const isOn = (ccValue >= 64); button.setCurrentValue?.(isOn ? 1.0 : 0.0); }
                }
            }
        });
